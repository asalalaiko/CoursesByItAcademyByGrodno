...
public class SQL {
	...
	public static final String SELECT_ALL_AND_DEP = "SELECT * FROM public.testtable t LEFT OUTER JOIN public.dept d ON t.dept_num = d.id order by t.id";
	public static final String SELECT_BY_ID = "select * from public.testtable where id = ?";
	public static final String SELECT_DEP_BY_ID = "select * from public.dept where id = ?";
	...
	public static final String UPDATE_DEP = "UPDATE public.dept SET "
			+ "dept_name = ?, dept_description = ? where id = ?";
